...
`scripts/Install-TechnicianWorkspaceGuestAgent.ps1` now provides a network-free Audit and a separately guarded Apply path for the signed 64-bit QEMU Guest Agent on the attached VirtIO ISO. It requires one exact installer candidate, Valid Authenticode status, the Red Hat signer subject, full Windows, elevation, an exact install phrase, `ShouldProcess`, no restart, and installed/running/automatic service verification. Fixtures cannot authorize installation. The live run remains pending OS installation.
