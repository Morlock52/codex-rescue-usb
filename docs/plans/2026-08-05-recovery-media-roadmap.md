...
**Status:** The historical alpha.4, alpha.7, and alpha.9 WinPE evidence layers remain VM-verified. The next candidate adds an exact destination-approval token, full destination rescan, a regular-file marker check, and a ten-file package with privacy-preserving offline Windows, setup/repair-log, offline-BCD, Autopilot, and Intune/MDM inventory. Its public file contract passed the full-Windows VM against both a disposable mapped installation and the real Windows installation: offline BCD enumeration, real Autopilot and MDM Admin log sampling, profile-name withholding, no-overwrite, valid summary creation, and missing-privacy-flag rejection all passed. Integration into a freshly built WinPE ISO and a disconnected WinPE boot/export remain pending, so this candidate is not yet promoted to the verified image list.
...
**Build work:** build the current candidate, confirm its exact destination approval and rescan in WinPE, export and independently verify the ten-file package, and verify that inaccessible candidate drives do not abort collection. Destination discovery must continue to reject WinPE's RAM drive and the ordinary internal system drive; source discovery may probe mounted Windows installations read-only.
