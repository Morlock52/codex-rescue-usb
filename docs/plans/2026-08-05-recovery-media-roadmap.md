...
**Status:** Partially VM-verified on August 5, 2026. The exact alpha.7 ISO completed the external `.bek` recovery-key sub-gate against one disposable encrypted data disk in an isolated Proxmox VM. A first alpha.10 candidate booted in the disconnected UEFI VM, but its displayed PowerShell command was blocked by WinPE execution policy before reaching the recovery script. The exact derived alpha.11 candidate (`558,899,200` bytes; SHA-256 `7EFB41B96A247FEB49E9B9037AD379F6528EC9184A105D19AF819532152513B0`) contains only the committed `startnet.cmd` correction over the boot-verified alpha.10 WIM. Its WIM verification, BIOS/UEFI catalog, disconnected UEFI boot in dedicated no-data-disk VM 114, corrected launch command, wrong-token refusal, and blocked `C:`/`X:` parameter refusals are verified. A clean ADK rebuild and the encrypted disposable-volume invalid-format, wrong-password, correct-unlock, fixture-file, cold-relock, and leakage gates remain pending. Operating-system-volume recovery, physical hardware, and production-data gates also remain pending.
